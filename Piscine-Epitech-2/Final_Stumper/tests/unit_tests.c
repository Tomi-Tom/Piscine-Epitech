...
Test(find_size, find_a_small_size)
{
    ...
    cr_assert_str_eq(find_size(tab, 1), "5 5\n");
}
...
Test(find_size, find_a_large_size)
{
    ...
    tab = my_str_to_word_array(str);
    cr_assert_str_eq(find_size(tab, 1), "10 10\n");
}
...
Test(find_size, find_the_smallest_size)
{
    ...
    tab = my_str_to_word_array(str);
    cr_assert_str_eq(find_size(tab, 1), "1 1\n");
}
...
Test(find_type, find_every_rush_with_1_1_B)
{
    ...
    tab = my_str_to_word_array(str);
    cr_assert_str_eq(find_type(tab), "[rush1-3] 2 1 || [rush1-4] 2 1 || [rush1-5] ");
}
